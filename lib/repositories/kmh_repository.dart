...
class KmhRepository {
  ...
  static const Duration _transactionCacheDuration = Duration(minutes: 2);
  Future<void> addTransaction(KmhTransaction transaction) async {
    await _box.put(transaction.id, transaction);
    CacheKeys.clearKmhCache(transaction.walletId);
  }
  Future<List<KmhTransaction>> getTransactions(String walletId) async {
    final cacheKey = CacheKeys.kmhTransactions(walletId);
    final cached = _cache.get<List<KmhTransaction>>(cacheKey);
    if (cached != null) {
      return cached;
    }
    ...
    _cache.set(cacheKey, transactions, duration: _transactionCacheDuration);
  ...
  Future<List<KmhTransaction>> getTransactionsByDateRange(
    ...
    final dateRange = '${start.millisecondsSinceEpoch}_${end.millisecondsSinceEpoch}';
