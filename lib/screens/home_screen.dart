...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  Future<void> _loadData() async {
    try {
      ...
      try { loadedWallets = await _dataService.getWallets(); } catch (e) { debugPrint('WARN _loadData: wallets $e'); }
      try { loadedGoals = await _dataService.getGoals(); } catch (e) { debugPrint('WARN _loadData: goals $e'); }
      try { loadedTransactions = await _dataService.getTransactions(); } catch (e) { debugPrint('WARN _loadData: transactions $e'); }
      try {
        final cats = await _dataService.getCategories();
        loadedCategories = List<Category>.from(cats);
      } catch (e) { debugPrint('WARN _loadData: categories $e'); }
      try { loadedLoans = await _dataService.getLoans(); } catch (e) { debugPrint('WARN _loadData: loans $e'); }
      try { unreadCount = await _notificationService.getUnreadCount(); } catch (e) { debugPrint('WARN _loadData: notifications $e'); }
      try { kmhAlerts = await _kmhAlertService.checkAllAccountsForAlerts(); } catch (e) { debugPrint('WARN _loadData: kmh alerts $e'); }
      try { cards = await _creditCardService.getAllCards(); } catch (e) { debugPrint('WARN _loadData: cards $e'); }
      ...
      for (var card in cards) {
        ...
        try {
          final ccTransactions = await _creditCardService.getCardTransactions(card.id);
          debugPrint('DEBUG _loadData: Card ${card.bankName} has ${ccTransactions.length} transactions');
          allCCTransactions.addAll(ccTransactions);
        } catch (e) {
          debugPrint('WARN _loadData: card tx for ${card.id} $e');
        }
      }
      debugPrint('DEBUG _loadData: Total CC transactions: ${allCCTransactions.length}');
            ...
            child: Row(
              ...
              children: [
                ...
                if (kmhDebts > 0) ...[
                  ...
                  _buildDebtStat('KMH', kmhDebts, Icons.account_balance),
                ],
                if (loanDebts > 0) ...[
                  ...
                  _buildDebtStat('Krediler', loanDebts, Icons.payments),
                ],
                if (totalDebts == 0)
                  const Expanded(
                    child: Text(
