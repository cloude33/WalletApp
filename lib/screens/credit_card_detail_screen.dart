...
class _CreditCardDetailScreenState extends State<CreditCardDetailScreen> {
  ...
  Future<void> _loadCardDetails() async {
    ...
    try {
      ...
      // Load reward points summary
      Map<String, dynamic>? rewardSummary;
      try {
        rewardSummary = await _rewardService.getPointsSummary(widget.card.id);
      } catch (e) {
        // Reward points may not be initialized for this card
        rewardSummary = null;
      }

      // Load cash advance summary
      ...
      // Calculate period debt and total debt
  ...
  void _navigateToInstallments() {
    // Show all installments in a list - could be implemented as a separate screen
    // For now, users can tap individual installments to see details
  ...
  Future<void> _navigateToRewardPoints() async {
    ...
    // Reload to reflect any changes in points
    _loadCardDetails();
  }
}
