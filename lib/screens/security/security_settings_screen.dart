...
class _SecuritySettingsScreenState extends State<SecuritySettingsScreen> {
        ...
        child: Column(
          children: [
            ...
            _buildSettingTile(
              icon: Icons.pin,
              title: 'PIN Kodu',
              subtitle: _securityConfig!.isPINEnabled
                  ? 'Etkin - Güvenli PIN koruması'
                  : 'Devre dışı',
              trailing: Switch(
                value: _securityConfig!.isPINEnabled,
                onChanged: (value) => _togglePINAuthentication(value),
            ...
            // PIN Change Option (only if PIN is enabled)
            if (_securityConfig!.isPINEnabled) ...[
              _buildActionTile(
                icon: Icons.edit,
  ...
  String _getSecurityLevelText() {
    ...
    if (_securityConfig!.isPINEnabled) score += 30;
    if (_securityConfig!.isBiometricEnabled) score += 25;
    if (_twoFactorConfig?.isEnabled == true) score += 35;
    if (_securityConfig!.sessionConfig.enableBackgroundLock) score += 10;
  ...
  Future<void> _togglePINAuthentication(bool enabled) async {
    try {
      if (enabled) {
        // Enable PIN - navigate to PIN setup
        ...
        final confirmed = await _showConfirmationDialog(
          title: 'PIN Kodunu Devre Dışı Bırak',
          message:
              'PIN kodu devre dışı bırakılacak. Bu işlem geri alınamaz. Devam etmek istiyor musunuz?',
          confirmText: 'Devre Dışı Bırak',
          isDestructive: true,
        );

        if (confirmed) {
          await _pinService.clearAllPINData();
          await _updateSecurityConfig(
            _securityConfig!.copyWith(isPINEnabled: false),
          );
        }
      }
    } catch (e) {
      _showErrorSnackBar('PIN ayarı değiştirilemedi: ${e.toString()}');
    }
  }
  ...
  Future<void> _changePIN() async {
    try {
      final result = await Navigator.of(
        context,
      ).push(MaterialPageRoute(builder: (context) => const PINChangeScreen()));

      if (result == true) {
        _showSuccessSnackBar('PIN kodu başarıyla değiştirildi');
      }
    } catch (e) {
      _showErrorSnackBar('PIN değiştirme ekranı açılamadı: ${e.toString()}');
    }
  }
