...
/// Global güvenlik state management provider
/// 
/// Bu provider, tüm güvenlik ile ilgili state'leri yönetir ve
/// UI bileşenlerine reactive bir arayüz sağlar.
/// 
/// Özellikler:
/// - Global güvenlik durumu yönetimi
/// - Authentication state tracking
/// - Security event stream yönetimi
/// - Servis koordinasyonu
/// - Reactive UI güncellemeleri
/// 
/// Implements Requirements:
/// - 1.4: PIN başarıyla doğrulandığında kullanıcı oturumu başlatmalı
/// - 6.2: 5 dakika boyunca aktivite olmadığında oturumu sonlandırmalı
/// - 7.5: Güvenlik ayarları değiştirildiğinde değişiklikleri audit loguna kaydetmeli
  ...
  Future<AuthResult> authenticateWithPIN(String pin) async {
    try {
      ...
      if (result.isSuccess) {
        ...
        debugPrint('SecurityProvider: PIN authentication successful');
      } else {
        _setError(result.errorMessage ?? 'PIN doğrulama başarısız');
      }

      return result;
    } catch (e) {
      _setError('PIN doğrulama hatası: ${e.toString()}');
  ...
  Future<void> _logSecurityConfigChange(
    ...
    try {
      ...
      if (oldConfig == null) {
        ...
        // PIN enabled değişikliği
        if (oldConfig.isPINEnabled != newConfig.isPINEnabled) {
          await logSecurityEvent(
            SecurityEvent.securitySettingsChanged(
              setting: 'pin_enabled',
              oldValue: oldConfig.isPINEnabled.toString(),
              newValue: newConfig.isPINEnabled.toString(),
            ),
          );
        }
