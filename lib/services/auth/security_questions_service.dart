...
/// 
/// Bu servis güvenlik sorularının oluşturulması, doğrulanması ve
/// PIN kurtarma sürecinin yönetilmesini sağlar.
/// 
/// Özellikler:
...
class SecurityQuestionsService {
  ...
  /// PIN kurtarma için rastgele güvenlik soruları seçer
  /// 
  ...
  /// PIN kurtarma durumunu başlatır
  Future<PINRecoveryState> startRecovery() async {
    try {
      await _ensureInitialized();
      
      final recoveryState = PINRecoveryState(
        isActive: true,
        currentStep: PINRecoveryStep.securityQuestions,
        startTime: DateTime.now(),
        lastActivity: DateTime.now(),
      );
      
      // Kurtarma durumunu depola
      await _storage.storePINRecoveryState(jsonEncode(recoveryState.toJson()));
      
      return recoveryState;
    } catch (e) {
      debugPrint('Failed to start recovery: $e');
      rethrow;
    }
  }

  /// PIN kurtarma durumunu günceller
  Future<PINRecoveryState> updateRecoveryState(PINRecoveryState state) async {
    try {
      await _ensureInitialized();
      ...
      // Güncellenmiş durumu depola
      await _storage.storePINRecoveryState(jsonEncode(updatedState.toJson()));
      
      return updatedState;
    } catch (e) {
      debugPrint('Failed to update recovery state: $e');
      rethrow;
    }
  }

  /// Mevcut PIN kurtarma durumunu alır
  Future<PINRecoveryState?> getRecoveryState() async {
    try {
      await _ensureInitialized();
      
      final stateJson = await _storage.getPINRecoveryState();
      if (stateJson == null) {
        return null;
      }
      
      return PINRecoveryState.fromJson(jsonDecode(stateJson));
    } catch (e) {
      debugPrint('Failed to get recovery state: $e');
      return null;
    }
  }

  /// PIN kurtarma durumunu temizler
  Future<void> clearRecoveryState() async {
    try {
      await _ensureInitialized();
      await _storage.clearPINRecoveryState();
    } catch (e) {
      debugPrint('Failed to clear recovery state: $e');
    }
  }
