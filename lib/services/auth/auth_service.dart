  ...
  Future<void> onAppForeground() async {
    try {
      ...
      if (backgroundTimestamp != null) {
        ...
        // Eğer arka planda geçen süre threshold'u aşmışsa logout yap
        if (elapsedTime >= config.sessionConfig.backgroundLockDelay) {
          debugPrint('Background time exceeded threshold, logging out');
          await logout();
          return;
        }
        
        // Threshold içindeyse timestamp'i temizle
        _backgroundTimestamp = null;
        await _storage.clearBackgroundTimestamp();
        debugPrint('Background lock timer cancelled, session maintained');
      }
    } catch (e) {
    ...
    try {
      ...
      if (backgroundTimestamp != null) {
        ...
        if (elapsedTime >= config.sessionConfig.backgroundLockDelay) {
          ...
          debugPrint('Background time exceeded threshold on app restart, clearing session');
          await _clearStoredAuthState();
          await _storage.clearBackgroundTimestamp();
          return;
        }
