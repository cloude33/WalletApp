import 'dart:developer' as developer;
import '../exceptions/credit_card_exception.dart';

/// Service for logging errors and debugging information
class ErrorLoggerService {
  static final ErrorLoggerService _instance = ErrorLoggerService._internal();
  factory ErrorLoggerService() => _instance;
  ErrorLoggerService._internal();

  /// Log levels
  static const String _levelError = 'ERROR';
  static const String _levelWarning = 'WARNING';
  static const String _levelInfo = 'INFO';
  static const String _levelDebug = 'DEBUG';

  /// Log an error
  void error(
    String message, {
    dynamic error,
    StackTrace? stackTrace,
    String? context,
  }) {
    _log(
      level: _levelError,
      message: message,
      error: error,
      stackTrace: stackTrace,
      context: context,
    );
  }

  /// Log a warning
  void warning(
    String message, {
    dynamic details,
    String? context,
  }) {
    _log(
      level: _levelWarning,
      message: message,
      error: details,
      context: context,
    );
  }

  /// Log an info message
  void info(
    String message, {
    dynamic details,
    String? context,
  }) {
    _log(
      level: _levelInfo,
      message: message,
      error: details,
      context: context,
    );
  }

  /// Log a debug message
  void debug(
    String message, {
    dynamic details,
    String? context,
  }) {
    _log(
      level: _levelDebug,
      message: message,
      error: details,
      context: context,
    );
  }

  /// Internal logging method
  void _log({
    required String level,
    required String message,
    dynamic error,
    StackTrace? stackTrace,
    String? context,
  }) {
    final timestamp = DateTime.now().toIso8601String();
    final contextStr = context != null ? '[$context] ' : '';
    final logMessage = '$timestamp [$level] $contextStr$message';

    // Log to console using developer.log for better debugging
    developer.log(
      logMessage,
      name: 'CreditCardApp',
      error: error,
      stackTrace: stackTrace,
      level: _getLogLevel(level),
    );

    // In production, you could also:
    // - Write to a file
    // - Send to a remote logging service
    // - Store in a local database for later analysis
  }

  /// Get numeric log level for developer.log
  int _getLogLevel(String level) {
    switch (level) {
      case _levelError:
        return 1000; // Severe
      case _levelWarning:
        return 900; // Warning
      case _levelInfo:
        return 800; // Info
      case _levelDebug:
        return 700; // Config/Debug
      default:
        return 500; // Fine
    }
  }

  /// Log a CreditCardException with full details
  void logException(
    CreditCardException exception, {
    String? context,
  }) {
    error(
      exception.message,
      error: exception,
      stackTrace: exception.stackTrace,
      context: context ?? 'CreditCardException',
    );
  }

  /// Log a generic exception
  void logGenericException(
    dynamic exception, {
    StackTrace? stackTrace,
    String? context,
  }) {
    error(
      exception.toString(),
      error: exception,
      stackTrace: stackTrace,
      context: context ?? 'Exception',
    );
  }

  /// Log a business logic validation error
  void logValidationError(
    String message, {
    String? field,
    dynamic value,
    String? context,
  }) {
    warning(
      message,
      details: {
        'field': field,
        'value': value,
      },
      context: context ?? 'Validation',
    );
  }

  /// Log a database operation
  void logDatabaseOperation(
    String operation,
    String entity, {
    bool success = true,
    dynamic error,
    String? details,
  }) {
    if (success) {
      debug(
        '$operation $entity',
        details: details,
        context: 'Database',
      );
    } else {
      error(
        'Failed to $operation $entity',
        error: error,
        context: 'Database',
      );
    }
  }

  /// Log a service operation
  void logServiceOperation(
    String service,
    String operation, {
    bool success = true,
    dynamic error,
    String? details,
  }) {
    if (success) {
      debug(
        '$operation in $service',
        details: details,
        context: service,
      );
    } else {
      error(
        'Failed $operation in $service',
        error: error,
        context: service,
      );
    }
  }
}
