...
class SecurityEvent {
  ...
  /// PIN oluşturma olayı
  factory SecurityEvent.pinCreated({
    String? userId,
    Map<String, dynamic>? metadata,
  }) {
    return SecurityEvent(
      type: SecurityEventType.pinCreated,
      userId: userId,
      description: 'PIN kodu oluşturuldu',
  ...
  /// PIN değiştirme olayı
  factory SecurityEvent.pinChanged({
    String? userId,
    Map<String, dynamic>? metadata,
  }) {
    return SecurityEvent(
      type: SecurityEventType.pinChanged,
      userId: userId,
      description: 'PIN kodu değiştirildi',
      severity: SecurityEventSeverity.info,
      source: 'PINService',
      metadata: metadata ?? {},
    );
  ...
  /// PIN doğrulama başarılı olayı
  factory SecurityEvent.pinVerified({
    String? userId,
    Map<String, dynamic>? metadata,
  }) {
    return SecurityEvent(
      type: SecurityEventType.pinVerified,
      userId: userId,
      description: 'PIN kodu başarıyla doğrulandı',
      severity: SecurityEventSeverity.info,
      source: 'PINService',
      metadata: metadata ?? {},
    );
  ...
  /// PIN doğrulama başarısız olayı
  factory SecurityEvent.pinFailed({
    String? userId,
    int? remainingAttempts,
    Map<String, dynamic>? metadata,
  }) {
    return SecurityEvent(
      type: SecurityEventType.pinFailed,
      userId: userId,
      description: 'PIN kodu doğrulama başarısız (kalan: ${remainingAttempts ?? 0})',
