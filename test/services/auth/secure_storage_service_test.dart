...
void main() {
  ...
  group('AuthSecureStorageService', () {
    ...
    test('should validate PIN format correctly', () async {
      // Test PIN validation logic without storage
      expect(''.length >= 4 && ''.length <= 6, isFalse); // Empty
      expect('123'.length >= 4 && '123'.length <= 6, isFalse); // Too short
      expect(
        '1234567'.length >= 4 && '1234567'.length <= 6,
        isFalse,
      ); // Too long
      expect(
        '1234'.length >= 4 && '1234'.length <= 6,
        isTrue,
      ); // Valid 4 digits
      expect(
        '12345'.length >= 4 && '12345'.length <= 6,
        isTrue,
      ); // Valid 5 digits
      expect(
        '123456'.length >= 4 && '123456'.length <= 6,
        isTrue,
      ); // Valid 6 digits
    });
    ...
    test(
      ...
      () async {
        ...
        expect(pinSet, isFalse); // Should return false when storage unavailable
    ...
    test(
      'should handle PIN operations gracefully when storage unavailable',
      () async {
        // Test PIN operations when storage is not available
        final storeResult = await service.storePIN('1234');
        expect(
          storeResult,
          isFalse,
        ); // Should return false when storage unavailable

        final verifyResult = await service.verifyPIN('1234');
        expect(
          verifyResult,
          isFalse,
        ); // Should return false when storage unavailable

        final removeResult = await service.removePIN();
        expect(
          removeResult,
          isFalse,
        ); // Should return false when storage unavailable
