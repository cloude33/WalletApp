...
void main() {
  ...
  group('SecurityProvider', () {
    ...
    test('should broadcast security events when logged', () async {
      ...
      final testEvent = SecurityEvent.pinCreated(
        userId: 'test_user',
        metadata: {'test': true},
      );
      ...
      expect(events.first.type, equals(SecurityEventType.pinCreated));
    ...
    test('should add logged events to recent events list', () async {
      ...
      final testEvent = SecurityEvent.pinVerified(
        userId: 'test_user',
      );
      ...
      expect(provider.recentEvents.first.type, equals(SecurityEventType.pinVerified));
    });
    ...
    test('should limit recent events to 50', () async {
      ...
      for (int i = 0; i < 60; i++) {
        await provider.logSecurityEvent(
          SecurityEvent.pinVerified(userId: 'user_$i'),
        );
      }
    ...
    test('should reset state for testing', () async {
      ...
      // Add some state
      await provider.logSecurityEvent(
        SecurityEvent.pinCreated(userId: 'test'),
      );
