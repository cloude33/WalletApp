...
/// Property-based tests for security settings persistence
/// 
/// **Feature: pin-biometric-auth, Property 7: Güvenlik Ayarları Kalıcılığı**
/// **Validates: Requirements 7.5**
/// 
/// This test verifies that any security setting change is persisted and
/// preserved when the application is restarted.
void main() {
...
SecurityConfig _generateRandomSecurityConfig() {
  return SecurityConfig(
    isPINEnabled: PropertyTest.randomBool(),
    ...
    maxPINAttempts: PropertyTest.randomInt(min: 3, max: 10),
    enabledBiometrics: _generateRandomBiometricTypes(),
    pinConfig: _generateRandomPINConfig(),
...
PINConfiguration _generateRandomPINConfig() {
  final minLength = PropertyTest.randomInt(min: 4, max: 6);
  final maxLength = PropertyTest.randomInt(min: minLength, max: 8);
  
  return PINConfiguration(
    minLength: minLength,
    maxLength: maxLength,
    maxAttempts: PropertyTest.randomInt(min: 3, max: 10),
    lockoutDuration: Duration(
      seconds: PropertyTest.randomInt(min: 30, max: 3600),
    ),
    requireComplexPIN: PropertyTest.randomBool(),
  );
}
...
BiometricConfiguration _generateRandomBiometricConfig() {
  return BiometricConfiguration(
    requireFallbackPIN: PropertyTest.randomBool(),
...
Map<String, dynamic> _generateSecuritySettingChange() {
  final settings = [
    {'setting': 'isPINEnabled', 'value': PropertyTest.randomBool()},
    ...
    {'setting': 'maxPINAttempts', 'value': PropertyTest.randomInt(min: 3, max: 10)},
...
SecurityConfig _applySettingChange(SecurityConfig config, String setting, dynamic value) {
  switch (setting) {
    case 'isPINEnabled':
      return config.copyWith(isPINEnabled: value as bool);
    ...
    case 'maxPINAttempts':
      return config.copyWith(maxPINAttempts: value as int);
...
bool _verifySettingValue(SecurityConfig config, String setting, dynamic expectedValue) {
  switch (setting) {
    case 'isPINEnabled':
      return config.isPINEnabled == expectedValue;
    case 'isBiometricEnabled':
    ...
    case 'maxPINAttempts':
      return config.maxPINAttempts == expectedValue;
    case 'enableBackgroundLock':
...
bool _configsAreEqual(SecurityConfig config1, SecurityConfig config2) {
  return config1.isPINEnabled == config2.isPINEnabled &&
         config1.isBiometricEnabled == config2.isBiometricEnabled &&
         config1.isTwoFactorEnabled == config2.isTwoFactorEnabled &&
         config1.sessionTimeout == config2.sessionTimeout &&
         config1.maxPINAttempts == config2.maxPINAttempts &&
         _biometricListsEqual(config1.enabledBiometrics, config2.enabledBiometrics) &&
         _pinConfigsEqual(config1.pinConfig, config2.pinConfig) &&
...
/// Compare two PIN configurations for equality
bool _pinConfigsEqual(PINConfiguration config1, PINConfiguration config2) {
  return config1.minLength == config2.minLength &&
         config1.maxLength == config2.maxLength &&
         config1.maxAttempts == config2.maxAttempts &&
         config1.lockoutDuration == config2.lockoutDuration &&
         config1.requireComplexPIN == config2.requireComplexPIN;
}
...
bool _biometricConfigsEqual(BiometricConfiguration config1, BiometricConfiguration config2) {
  return config1.requireFallbackPIN == config2.requireFallbackPIN &&
         config1.maxAttempts == config2.maxAttempts &&
