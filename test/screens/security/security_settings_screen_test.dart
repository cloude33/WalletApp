...
void main() {
  group('SecuritySettingsScreen Widget Tests', () {
    ...
    testWidgets('should show PIN toggle switch', (WidgetTester tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: SecuritySettingsScreen(),
        ),
      ...
      // Find PIN toggle switch
      final pinSwitches = find.byType(Switch);
      expect(pinSwitches, findsWidgets);
    });
    ...
    testWidgets('should display PIN change option when PIN is enabled', (WidgetTester tester) async {
      // Enable PIN first
      ...
      // Verify PIN change option is displayed
      expect(find.text('PIN Kodunu Değiştir'), findsOneWidget);
      expect(find.text('Mevcut PIN kodunuzu değiştirin'), findsOneWidget);
    });
    ...
    testWidgets('should navigate to PIN change screen when tapped', (WidgetTester tester) async {
      // Enable PIN first
      await pinService.setupPIN('1234');
      ...
      await tester.pumpWidget(
        MaterialApp(
          ...
          routes: {
            '/pin-change': (context) => const Scaffold(
              body: Center(child: Text('PIN Change Screen')),
            ),
          },
        ),
      );
      ...
      // Find and tap PIN change option
      final pinChangeButton = find.text('PIN Kodunu Değiştir');
      if (pinChangeButton.evaluate().isNotEmpty) {
        await tester.tap(pinChangeButton);
        await tester.pumpAndSettle();
      }
    });
  ...
  group('SecuritySettingsScreen Integration Tests', () {
    testWidgets('should update UI when PIN is enabled', (WidgetTester tester) async {
      ...
      // Initially PIN should be disabled
      // Enable PIN
      await pinService.setupPIN('1234');
