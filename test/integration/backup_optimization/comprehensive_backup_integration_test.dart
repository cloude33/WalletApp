import 'dart:convert';

import 'package:flutter_test/flutter_test.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:archive/archive.dart';

import 'package:parion/services/backup_service.dart';
import 'package:parion/services/data_service.dart';
import 'package:parion/models/transaction.dart';
import 'package:parion/models/wallet.dart';

import '../../test_setup.dart';
import '../../test_helpers.dart';

void main() {
  setupCommonTestMocks();

  group('Comprehensive Backup Integration Tests - Task 12.2', () {
    late BackupService backupService;
    late DataService dataService;

    /// Helper method to restore from backup bytes
    Future<void> restoreFromBackupBytes(List<int> backupBytes) async {
      final decompressed = GZipDecoder().decodeBytes(backupBytes);
      final jsonString = utf8.decode(decompressed);
      final backupData = jsonDecode(jsonString) as Map<String, dynamic>;
      await dataService.restoreFromBackup(backupData);
    }

    setUpAll(() async {
      await TestSetup.initializeTestEnvironment();

      // Initialize services
      backupService = BackupService();
      dataService = DataService();

      await dataService.init();
      await backupService.loadSettings();
    });

    tearDownAll(() async {
      await TestSetup.cleanupTestEnvironment();
    });

    setUp(() async {
      await TestSetup.setupTest();

      // Clear preferences for clean test state
      final prefs = await SharedPreferences.getInstance();
      await prefs.clear();
    });

    tearDown(() async {
      await TestSetup.tearDownTest();
    });

    group('End-to-End Backup and Restore Workflows', () {
      test(
        'Complete backup and restore workflow with comprehensive data',
        () async {
          try {
            // Create comprehensive test dataset
            final testWallets = _createTestWallets(15);
            final testTransactions = _createTestTransactions(100, testWallets);

            // Save test data
            await dataService.saveWallets(testWallets);
            await dataService.saveTransactions(testTransactions);

            // Verify data is saved
            final savedWallets = await dataService.getWallets();
            final savedTransactions = await dataService.getTransactions();
            expect(savedWallets.length, testWallets.length);
            expect(savedTransactions.length, testTransactions.length);

            print(
              '✅ Test data created: ${testWallets.length} wallets, ${testTransactions.length} transactions',
            );

            // Create backup
            final backupData = await backupService.createBackupRaw();
            expect(backupData, isNotNull);
            expect(backupData.length, greaterThan(0));

            // Verify backup metadata
            final metadata = await backupService.getBackupMetadataFromBytes(
              backupData,
            );
            expect(metadata, isNotNull);
            expect(metadata!.walletCount, testWallets.length);
            expect(metadata.transactionCount, testTransactions.length);
            expect(metadata.version, isNotNull);
            expect(metadata.createdAt, isNotNull);

            print('✅ Backup created successfully');
            print('   - Backup size: ${backupData.length} bytes');
            print('   - Wallets in backup: ${metadata.walletCount}');
            print('   - Transactions in backup: ${metadata.transactionCount}');
            print('   - Backup version: ${metadata.version}');

            // Clear all data
            await dataService.saveWallets([]);
            await dataService.saveTransactions([]);

            // Verify data is cleared
            final clearedWallets = await dataService.getWallets();
            final clearedTransactions = await dataService.getTransactions();
            expect(clearedWallets.length, 0);
            expect(clearedTransactions.length, 0);

            print('✅ Data cleared for restore test');

            // Restore from backup
            await restoreFromBackupBytes(backupData);

            // Verify data is restored
            final restoredWallets = await dataService.getWallets();
            final restoredTransactions = await dataService.getTransactions();

            expect(restoredWallets.length, testWallets.length);
            expect(restoredTransactions.length, testTransactions.length);

            // Verify data integrity
            for (final originalWallet in testWallets) {
              final found = restoredWallets.firstWhere(
                (w) => w.id == originalWallet.id,
              );
              expect(found.name, originalWallet.name);
              expect(found.balance, originalWallet.balance);
              expect(found.type, originalWallet.type);
              expect(found.color, originalWallet.color);
              expect(found.icon, originalWallet.icon);
            }

            for (final originalTransaction in testTransactions) {
              final found = restoredTransactions.firstWhere(
                (t) => t.id == originalTransaction.id,
              );
              expect(found.description, originalTransaction.description);
              expect(found.amount, originalTransaction.amount);
              expect(found.category, originalTransaction.category);
              expect(found.type, originalTransaction.type);
              expect(found.walletId, originalTransaction.walletId);
            }

            print('✅ Full backup and restore workflow completed successfully');
          } catch (e, stackTrace) {
            print('❌ Full backup and restore workflow error: $e');
            print('Stack trace: $stackTrace');
            fail('Full backup and restore workflow failed: $e');
          }
        },
      );

      test('Incremental backup simulation with data changes', () async {
        try {
          // Create initial dataset
          final initialWallets = _createTestWallets(10);
          final initialTransactions = _createTestTransactions(
            50,
            initialWallets,
          );

          await dataService.saveWallets(initialWallets);
          await dataService.saveTransactions(initialTransactions);

          // Create first backup
          final firstBackup = await backupService.createBackupRaw();
          expect(firstBackup, isNotNull);

          final firstMetadata = await backupService.getBackupMetadataFromBytes(
            firstBackup,
          );
          expect(firstMetadata!.walletCount, initialWallets.length);
          expect(firstMetadata.transactionCount, initialTransactions.length);

          print(
            '✅ First backup created: ${firstMetadata.walletCount} wallets, ${firstMetadata.transactionCount} transactions',
          );

          // Simulate data changes
          final additionalWallets = _createTestWallets(5, startId: 100);
          final additionalTransactions = _createTestTransactions(
            25,
            initialWallets + additionalWallets,
            startId: 200,
          );

          await dataService.saveWallets(initialWallets + additionalWallets);
          await dataService.saveTransactions(
            initialTransactions + additionalTransactions,
          );

          // Create second backup (simulating incremental)
          final secondBackup = await backupService.createBackupRaw();
          expect(secondBackup, isNotNull);

          final secondMetadata = await backupService.getBackupMetadataFromBytes(
            secondBackup,
          );
          expect(
            secondMetadata!.walletCount,
            initialWallets.length + additionalWallets.length,
          );
          expect(
            secondMetadata.transactionCount,
            initialTransactions.length + additionalTransactions.length,
          );

          print(
            '✅ Second backup created: ${secondMetadata.walletCount} wallets, ${secondMetadata.transactionCount} transactions',
          );

          // Clear data and restore from second backup
          await dataService.saveWallets([]);
          await dataService.saveTransactions([]);

          await restoreFromBackupBytes(secondBackup);

          // Verify all data is restored
          final restoredWallets = await dataService.getWallets();
          final restoredTransactions = await dataService.getTransactions();

          expect(
            restoredWallets.length,
            initialWallets.length + additionalWallets.length,
          );
          expect(
            restoredTransactions.length,
            initialTransactions.length + additionalTransactions.length,
          );

          print('✅ Incremental backup simulation completed successfully');
        } catch (e, stackTrace) {
          print('❌ Incremental backup simulation error: $e');
          print('Stack trace: $stackTrace');
          fail('Incremental backup simulation failed: $e');
        }
      });

      test('Backup with special characters and unicode data', () async {
        try {
          // Create test data with special characters and unicode
          final specialWallets = [
            Wallet(
              id: 'unicode_1',
              name: 'Türkçe Cüzdan 💰',
              balance: 1234.56,
              type: 'bank',
              color: '#FF5722',
              icon: 'wallet',
            ),
            Wallet(
              id: 'unicode_2',
              name: '中文钱包 🏦',
              balance: 9876.54,
              type: 'cash',
              color: '#2196F3',
              icon: 'cash',
            ),
            Wallet(
              id: 'unicode_3',
              name: 'العربية محفظة 💳',
              balance: 5555.55,
              type: 'credit',
              color: '#4CAF50',
              icon: 'credit',
            ),
            Wallet(
              id: 'unicode_4',
              name: 'Русский кошелек 🪙',
              balance: 7777.77,
              type: 'savings',
              color: '#9C27B0',
              icon: 'savings',
            ),
          ];

          final specialTransactions = [
            Transaction(
              id: 'unicode_trans_1',
              amount: 123.45,
              description: 'Café ☕ & Résumé 📄',
              date: DateTime.now(),
              type: 'expense',
              walletId: 'unicode_1',
              category: 'Özel Kategori',
            ),
            Transaction(
              id: 'unicode_trans_2',
              amount: 678.90,
              description: '购物 🛒 和 餐厅 🍽️',
              date: DateTime.now(),
              type: 'expense',
              walletId: 'unicode_2',
              category: '购物',
            ),
            Transaction(
              id: 'unicode_trans_3',
              amount: 999.99,
              description: 'مطعم 🍕 و تسوق 🛍️',
              date: DateTime.now(),
              type: 'expense',
              walletId: 'unicode_3',
              category: 'مطاعم',
            ),
            Transaction(
              id: 'unicode_trans_4',
              amount: 555.55,
              description: 'Покупки 🛒 и кафе ☕',
              date: DateTime.now(),
              type: 'expense',
              walletId: 'unicode_4',
              category: 'Покупки',
            ),
          ];

          await dataService.saveWallets(specialWallets);
          await dataService.saveTransactions(specialTransactions);

          // Create backup
          final backupData = await backupService.createBackupRaw();
          expect(backupData, isNotNull);

          // Clear data
          await dataService.saveWallets([]);
          await dataService.saveTransactions([]);

          // Restore backup
          await restoreFromBackupBytes(backupData);

          // Verify unicode characters are preserved
          final restoredWallets = await dataService.getWallets();
          final restoredTransactions = await dataService.getTransactions();

          expect(restoredWallets.length, specialWallets.length);
          expect(restoredTransactions.length, specialTransactions.length);

          // Check each language's characters
          final turkishWallet = restoredWallets.firstWhere(
            (w) => w.id == 'unicode_1',
          );
          expect(turkishWallet.name, 'Türkçe Cüzdan 💰');

          final chineseWallet = restoredWallets.firstWhere(
            (w) => w.id == 'unicode_2',
          );
          expect(chineseWallet.name, '中文钱包 🏦');

          final arabicWallet = restoredWallets.firstWhere(
            (w) => w.id == 'unicode_3',
          );
          expect(arabicWallet.name, 'العربية محفظة 💳');

          final russianWallet = restoredWallets.firstWhere(
            (w) => w.id == 'unicode_4',
          );
          expect(russianWallet.name, 'Русский кошелек 🪙');

          // Check transaction descriptions and categories
          final turkishTransaction = restoredTransactions.firstWhere(
            (t) => t.id == 'unicode_trans_1',
          );
          expect(turkishTransaction.description, 'Café ☕ & Résumé 📄');
          expect(turkishTransaction.category, 'Özel Kategori');

          final chineseTransaction = restoredTransactions.firstWhere(
            (t) => t.id == 'unicode_trans_2',
          );
          expect(chineseTransaction.description, '购物 🛒 和 餐厅 🍽️');
          expect(chineseTransaction.category, '购物');

          final arabicTransaction = restoredTransactions.firstWhere(
            (t) => t.id == 'unicode_trans_3',
          );
          expect(arabicTransaction.description, 'مطعم 🍕 و تسوق 🛍️');
          expect(arabicTransaction.category, 'مطاعم');

          final russianTransaction = restoredTransactions.firstWhere(
            (t) => t.id == 'unicode_trans_4',
          );
          expect(russianTransaction.description, 'Покупки 🛒 и кафе ☕');
          expect(russianTransaction.category, 'Покупки');

          print(
            '✅ Unicode and special characters preserved across all languages',
          );
        } catch (e, stackTrace) {
          print('❌ Unicode preservation test error: $e');
          print('Stack trace: $stackTrace');
          fail('Unicode preservation test failed: $e');
        }
      });
    });

    group('Cross-Platform Compatibility Tests', () {
      test('Backup format compatibility and metadata validation', () async {
        try {
          // Create test data
          final testWallets = _createTestWallets(8);
          final testTransactions = _createTestTransactions(40, testWallets);

          await dataService.saveWallets(testWallets);
          await dataService.saveTransactions(testTransactions);

          // Create backup
          final backupData = await backupService.createBackupRaw();
          expect(backupData, isNotNull);

          // Extract and verify metadata
          final metadata = await backupService.getBackupMetadataFromBytes(
            backupData,
          );
          expect(metadata, isNotNull);

          // Verify cross-platform compatibility
          expect(metadata!.isCrossPlatformCompatible, true);
          expect(metadata.version, isNotNull);
          expect(metadata.version.isNotEmpty, true);

          // Platform information should be present
          expect(metadata.platform, isNotNull);
          expect(metadata.platform.isNotEmpty, true);
          expect(metadata.deviceModel, isNotNull);
          expect(metadata.deviceModel.isNotEmpty, true);

          // Verify backup structure
          final decompressed = GZipDecoder().decodeBytes(backupData);
          final jsonString = utf8.decode(decompressed);
          final backupStructure =
              jsonDecode(jsonString) as Map<String, dynamic>;

          // Check required backup structure elements
          expect(backupStructure.containsKey('metadata'), true);
          expect(backupStructure.containsKey('transactions'), true);
          expect(backupStructure.containsKey('wallets'), true);
          expect(backupStructure.containsKey('users'), true);

          print('✅ Cross-platform backup format verified');
          print('   - Platform: ${metadata.platform}');
          print('   - Device: ${metadata.deviceModel}');
          print('   - Version: ${metadata.version}');
          print(
            '   - Cross-platform compatible: ${metadata.isCrossPlatformCompatible}',
          );
          print('   - Backup structure validated');
        } catch (e, stackTrace) {
          print('❌ Cross-platform compatibility test error: $e');
          print('Stack trace: $stackTrace');
          fail('Cross-platform compatibility test failed: $e');
        }
      });

      test('Numeric precision preservation across platforms', () async {
        try {
          // Create test data with various numeric precisions
          final precisionTestWallets = [
            Wallet(
              id: 'precision_1',
              name: 'High Precision',
              balance: 123.456789,
              type: 'bank',
              color: '#2196F3',
              icon: 'wallet',
            ),
            Wallet(
              id: 'precision_2',
              name: 'Very Small',
              balance: 0.01,
              type: 'cash',
              color: '#4CAF50',
              icon: 'cash',
            ),
            Wallet(
              id: 'precision_3',
              name: 'Large Number',
              balance: 999999.99,
              type: 'savings',
              color: '#FF9800',
              icon: 'savings',
            ),
            Wallet(
              id: 'precision_4',
              name: 'Negative',
              balance: -1234.56,
              type: 'credit',
              color: '#F44336',
              icon: 'credit',
            ),
            Wallet(
              id: 'precision_5',
              name: 'Zero',
              balance: 0.0,
              type: 'bank',
              color: '#9C27B0',
              icon: 'wallet',
            ),
            Wallet(
              id: 'precision_6',
              name: 'Scientific',
              balance: 1.23e-4,
              type: 'cash',
              color: '#607D8B',
              icon: 'cash',
            ),
          ];

          final precisionTestTransactions = [
            Transaction(
              id: 'prec_trans_1',
              amount: 0.001,
              description: 'Micro Transaction',
              date: DateTime.now(),
              type: 'expense',
              walletId: 'precision_1',
              category: 'test',
            ),
            Transaction(
              id: 'prec_trans_2',
              amount: 1234567.89,
              description: 'Large Transaction',
              date: DateTime.now(),
              type: 'income',
              walletId: 'precision_2',
              category: 'test',
            ),
            Transaction(
              id: 'prec_trans_3',
              amount: -999.999,
              description: 'Negative Transaction',
              date: DateTime.now(),
              type: 'expense',
              walletId: 'precision_3',
              category: 'test',
            ),
            Transaction(
              id: 'prec_trans_4',
              amount: 0.0,
              description: 'Zero Transaction',
              date: DateTime.now(),
              type: 'income',
              walletId: 'precision_4',
              category: 'test',
            ),
            Transaction(
              id: 'prec_trans_5',
              amount: 3.14159265359,
              description: 'Pi Transaction',
              date: DateTime.now(),
              type: 'expense',
              walletId: 'precision_5',
              category: 'test',
            ),
            Transaction(
              id: 'prec_trans_6',
              amount: 1.23e-6,
              description: 'Scientific Transaction',
              date: DateTime.now(),
              type: 'income',
              walletId: 'precision_6',
              category: 'test',
            ),
          ];

          await dataService.saveWallets(precisionTestWallets);
          await dataService.saveTransactions(precisionTestTransactions);

          // Create backup
          final backupData = await backupService.createBackupRaw();
          expect(backupData, isNotNull);

          // Clear and restore
          await dataService.saveWallets([]);
          await dataService.saveTransactions([]);

          await restoreFromBackupBytes(backupData);

          final restoredWallets = await dataService.getWallets();
          final restoredTransactions = await dataService.getTransactions();

          expect(restoredWallets.length, precisionTestWallets.length);
          expect(restoredTransactions.length, precisionTestTransactions.length);

          // Verify numeric precision is preserved
          for (final original in precisionTestWallets) {
            final restored = restoredWallets.firstWhere(
              (w) => w.id == original.id,
            );
            expect(restored.balance, closeTo(original.balance, 0.000001));
            print(
              '✅ Wallet precision preserved: ${original.id} - ${original.balance} → ${restored.balance}',
            );
          }

          for (final original in precisionTestTransactions) {
            final restored = restoredTransactions.firstWhere(
              (t) => t.id == original.id,
            );
            expect(restored.amount, closeTo(original.amount, 0.000001));
            print(
              '✅ Transaction precision preserved: ${original.id} - ${original.amount} → ${restored.amount}',
            );
          }

          print(
            '✅ Numeric precision preservation verified across all test cases',
          );
        } catch (e, stackTrace) {
          print('❌ Numeric precision test error: $e');
          print('Stack trace: $stackTrace');
          fail('Numeric precision test failed: $e');
        }
      });

      test('Date and time preservation across time zones', () async {
        try {
          // Create test data with various dates and times
          final now = DateTime.now();
          final utcNow = DateTime.now().toUtc();
          final pastDate = DateTime(2020, 1, 15, 10, 30, 45);
          final futureDate = DateTime(2030, 12, 25, 23, 59, 59);

          final dateTestTransactions = [
            Transaction(
              id: 'date_trans_1',
              amount: 100.0,
              description: 'Current Local Time',
              date: now,
              type: 'expense',
              walletId: 'test_wallet',
              category: 'test',
            ),
            Transaction(
              id: 'date_trans_2',
              amount: 200.0,
              description: 'Current UTC Time',
              date: utcNow,
              type: 'income',
              walletId: 'test_wallet',
              category: 'test',
            ),
            Transaction(
              id: 'date_trans_3',
              amount: 300.0,
              description: 'Past Date',
              date: pastDate,
              type: 'expense',
              walletId: 'test_wallet',
              category: 'test',
            ),
            Transaction(
              id: 'date_trans_4',
              amount: 400.0,
              description: 'Future Date',
              date: futureDate,
              type: 'income',
              walletId: 'test_wallet',
              category: 'test',
            ),
          ];

          final testWallet = Wallet(
            id: 'test_wallet',
            name: 'Test Wallet',
            balance: 1000.0,
            type: 'bank',
            color: '#2196F3',
            icon: 'wallet',
          );

          await dataService.saveWallets([testWallet]);
          await dataService.saveTransactions(dateTestTransactions);

          // Create backup
          final backupData = await backupService.createBackupRaw();
          expect(backupData, isNotNull);

          // Clear and restore
          await dataService.saveWallets([]);
          await dataService.saveTransactions([]);

          await restoreFromBackupBytes(backupData);

          final restoredTransactions = await dataService.getTransactions();
          expect(restoredTransactions.length, dateTestTransactions.length);

          // Verify date and time preservation
          for (final original in dateTestTransactions) {
            final restored = restoredTransactions.firstWhere(
              (t) => t.id == original.id,
            );
            expect(
              restored.date.millisecondsSinceEpoch,
              original.date.millisecondsSinceEpoch,
            );
            print(
              '✅ Date preserved: ${original.id} - ${original.date} → ${restored.date}',
            );
          }

          print('✅ Date and time preservation verified across time zones');
        } catch (e, stackTrace) {
          print('❌ Date and time preservation test error: $e');
          print('Stack trace: $stackTrace');
          fail('Date and time preservation test failed: $e');
        }
      });
    });

    group('Performance and Scalability Tests', () {
      test('Large dataset backup and restore performance', () async {
        try {
          // Create large dataset
          final largeWalletList = _createTestWallets(100);
          final largeTransactionList = _createTestTransactions(
            1000,
            largeWalletList,
          );

          await dataService.saveWallets(largeWalletList);
          await dataService.saveTransactions(largeTransactionList);

          print(
            '✅ Large dataset created: ${largeWalletList.length} wallets, ${largeTransactionList.length} transactions',
          );

          // Measure backup performance
          final backupStopwatch = Stopwatch()..start();
          final backupData = await backupService.createBackupRaw();
          backupStopwatch.stop();

          expect(backupData, isNotNull);
          expect(backupData.length, greaterThan(0));
          expect(
            backupStopwatch.elapsed.inSeconds,
            lessThan(120),
          ); // Should complete within 2 minutes

          // Verify backup contains expected data
          final metadata = await backupService.getBackupMetadataFromBytes(
            backupData,
          );
          expect(metadata, isNotNull);
          expect(metadata!.walletCount, largeWalletList.length);
          expect(metadata.transactionCount, largeTransactionList.length);

          print(
            '✅ Large dataset backup completed in ${backupStopwatch.elapsedMilliseconds}ms',
          );
          print('   - Backup size: ${backupData.length} bytes');
          print(
            '   - Compression ratio: ${((largeWalletList.length + largeTransactionList.length) * 100 / backupData.length).toStringAsFixed(2)}%',
          );

          // Clear data
          await dataService.saveWallets([]);
          await dataService.saveTransactions([]);

          // Measure restore performance
          final restoreStopwatch = Stopwatch()..start();
          await restoreFromBackupBytes(backupData);
          restoreStopwatch.stop();

          expect(
            restoreStopwatch.elapsed.inSeconds,
            lessThan(120),
          ); // Should restore within 2 minutes

          // Verify data integrity
          final restoredWallets = await dataService.getWallets();
          final restoredTransactions = await dataService.getTransactions();

          expect(restoredWallets.length, largeWalletList.length);
          expect(restoredTransactions.length, largeTransactionList.length);

          print(
            '✅ Large dataset restore completed in ${restoreStopwatch.elapsedMilliseconds}ms',
          );
          print(
            '   - Total time: ${(backupStopwatch.elapsedMilliseconds + restoreStopwatch.elapsedMilliseconds)}ms',
          );
        } catch (e, stackTrace) {
          print('❌ Large dataset performance test error: $e');
          print('Stack trace: $stackTrace');
          fail('Large dataset performance test failed: $e');
        }
      });

      testWidgets('Multiple backup operations consistency', (
        WidgetTester tester,
      ) async {
        try {
          // Create test data
          final testWallets = _createTestWallets(20);
          final testTransactions = _createTestTransactions(100, testWallets);

          await dataService.saveWallets(testWallets);
          await dataService.saveTransactions(testTransactions);

          // Perform multiple backup operations
          final backupTimes = <Duration>[];
          final backupSizes = <int>[];

          for (int i = 0; i < 5; i++) {
            final stopwatch = Stopwatch()..start();
            final backupData = await backupService.createBackupRaw();
            stopwatch.stop();

            expect(backupData, isNotNull);
            expect(backupData.length, greaterThan(0));

            backupTimes.add(stopwatch.elapsed);
            backupSizes.add(backupData.length);

            // Each backup should complete within reasonable time
            expect(stopwatch.elapsed.inSeconds, lessThan(60));

            // Verify backup consistency
            final metadata = await backupService.getBackupMetadataFromBytes(
              backupData,
            );
            expect(metadata!.walletCount, testWallets.length);
            expect(metadata.transactionCount, testTransactions.length);
          }

          // Calculate statistics
          final averageTime =
              backupTimes.fold<Duration>(
                Duration.zero,
                (prev, time) => prev + time,
              ) ~/
              backupTimes.length;

          final averageSize =
              backupSizes.fold<int>(0, (prev, size) => prev + size) ~/
              backupSizes.length;

          // Verify consistency
          final maxSizeDifference =
              backupSizes.reduce((a, b) => a > b ? a : b) -
              backupSizes.reduce((a, b) => a < b ? a : b);
          expect(
            maxSizeDifference,
            lessThan(1000),
          ); // Size difference should be minimal

          print('✅ Multiple backup operations completed consistently');
          print('   - Average backup time: ${averageTime.inMilliseconds}ms');
          print('   - Average backup size: $averageSize bytes');
          print('   - Max size difference: $maxSizeDifference bytes');
          for (int i = 0; i < backupTimes.length; i++) {
            print(
              '   - Backup ${i + 1}: ${backupTimes[i].inMilliseconds}ms, ${backupSizes[i]} bytes',
            );
          }
        } catch (e, stackTrace) {
          print('❌ Multiple backup operations test error: $e');
          print('Stack trace: $stackTrace');
          fail('Multiple backup operations test failed: $e');
        }
      });
    });

    group('Error Handling and Edge Cases', () {
      test('Backup with empty data', () async {
        try {
          // Ensure no data exists
          await dataService.saveWallets([]);
          await dataService.saveTransactions([]);

          // Create backup with empty data
          final backupData = await backupService.createBackupRaw();
          expect(backupData, isNotNull);
          expect(backupData.length, greaterThan(0));

          // Verify backup metadata
          final metadata = await backupService.getBackupMetadataFromBytes(
            backupData,
          );
          expect(metadata, isNotNull);
          expect(metadata!.walletCount, 0);
          expect(metadata.transactionCount, 0);
          expect(metadata.version, isNotNull);

          // Verify backup can be restored
          await restoreFromBackupBytes(backupData);

          final restoredWallets = await dataService.getWallets();
          final restoredTransactions = await dataService.getTransactions();

          expect(restoredWallets.length, 0);
          expect(restoredTransactions.length, 0);

          print('✅ Empty data backup handled correctly');
        } catch (e, stackTrace) {
          print('❌ Empty data backup test error: $e');
          print('Stack trace: $stackTrace');
          fail('Empty data backup test failed: $e');
        }
      });

      test('Backup service configuration and status management', () async {
        try {
          // Test backup service configuration
          await backupService.enableAutoCloudBackup(true);
          expect(backupService.autoCloudBackupEnabled.value, true);

          await backupService.enableAutoCloudBackup(false);
          expect(backupService.autoCloudBackupEnabled.value, false);

          // Test status management
          backupService.cloudBackupStatus.value = CloudBackupStatus.uploading;
          expect(
            backupService.cloudBackupStatus.value,
            CloudBackupStatus.uploading,
          );

          backupService.cloudBackupStatus.value = CloudBackupStatus.downloading;
          expect(
            backupService.cloudBackupStatus.value,
            CloudBackupStatus.downloading,
          );

          backupService.cloudBackupStatus.value = CloudBackupStatus.error;
          expect(
            backupService.cloudBackupStatus.value,
            CloudBackupStatus.error,
          );

          // Test status text
          final statusText = backupService.getCloudBackupStatusText();
          expect(statusText, isNotNull);
          expect(statusText.isNotEmpty, true);

          // Reset status
          backupService.cloudBackupStatus.value = CloudBackupStatus.idle;
          expect(backupService.cloudBackupStatus.value, CloudBackupStatus.idle);

          print(
            '✅ Backup service configuration and status management verified',
          );
        } catch (e, stackTrace) {
          print('❌ Configuration and status test error: $e');
          print('Stack trace: $stackTrace');
          fail('Configuration and status test failed: $e');
        }
      });

      test('Backup metadata validation and version compatibility', () async {
        try {
          // Create test data
          final testWallets = _createTestWallets(5);
          final testTransactions = _createTestTransactions(10, testWallets);

          await dataService.saveWallets(testWallets);
          await dataService.saveTransactions(testTransactions);

          // Create backup
          final backupData = await backupService.createBackupRaw();
          expect(backupData, isNotNull);

          // Extract and validate metadata
          final metadata = await backupService.getBackupMetadataFromBytes(
            backupData,
          );
          expect(metadata, isNotNull);

          // Validate metadata fields
          expect(metadata!.version, isNotNull);
          expect(metadata.version.isNotEmpty, true);
          expect(metadata.createdAt, isNotNull);
          expect(metadata.walletCount, testWallets.length);
          expect(metadata.transactionCount, testTransactions.length);
          expect(metadata.platform, isNotNull);
          expect(metadata.platform.isNotEmpty, true);
          expect(metadata.deviceModel, isNotNull);
          expect(metadata.deviceModel.isNotEmpty, true);
          expect(metadata.isCrossPlatformCompatible, true);

          // Validate timestamp is recent
          final timeDifference = DateTime.now()
              .difference(metadata.createdAt)
              .inMinutes;
          expect(
            timeDifference,
            lessThan(5),
          ); // Should be created within last 5 minutes

          print('✅ Backup metadata validation completed');
          print('   - Version: ${metadata.version}');
          print('   - Created: ${metadata.createdAt}');
          print('   - Platform: ${metadata.platform}');
          print('   - Device: ${metadata.deviceModel}');
          print('   - Cross-platform: ${metadata.isCrossPlatformCompatible}');
        } catch (e, stackTrace) {
          print('❌ Metadata validation test error: $e');
          print('Stack trace: $stackTrace');
          fail('Metadata validation test failed: $e');
        }
      });
    });
  });
}

/// Helper function to create test wallets
List<Wallet> _createTestWallets(int count, {int startId = 0}) {
  final wallets = <Wallet>[];
  final types = ['bank', 'cash', 'credit', 'savings'];
  final colors = [
    '#FF5722',
    '#2196F3',
    '#4CAF50',
    '#FF9800',
    '#9C27B0',
    '#607D8B',
    '#795548',
    '#E91E63',
  ];
  final icons = ['wallet', 'cash', 'credit', 'savings', 'bank'];

  for (int i = 0; i < count; i++) {
    wallets.add(
      Wallet(
        id: 'integration_wallet_${startId + i}',
        name: 'Integration Test Wallet ${startId + i}',
        balance: (i + 1) * 100.0 + (i * 0.99), // Add some decimal variation
        type: types[i % types.length],
        color: colors[i % colors.length],
        icon: icons[i % icons.length],
      ),
    );
  }

  return wallets;
}

/// Helper function to create test transactions
List<Transaction> _createTestTransactions(
  int count,
  List<Wallet> wallets, {
  int startId = 0,
}) {
  final transactions = <Transaction>[];
  final categories = [
    'food',
    'transport',
    'entertainment',
    'shopping',
    'bills',
    'salary',
    'investment',
    'healthcare',
  ];
  final types = ['expense', 'income'];
  final descriptions = [
    'Grocery shopping',
    'Gas station',
    'Movie tickets',
    'Online purchase',
    'Electricity bill',
    'Monthly salary',
    'Stock dividend',
    'Doctor visit',
  ];

  for (int i = 0; i < count; i++) {
    transactions.add(
      Transaction(
        id: 'integration_transaction_${startId + i}',
        amount: (i + 1) * 10.0 + (i * 0.33), // Add some decimal variation
        description: '${descriptions[i % descriptions.length]} ${startId + i}',
        date: DateTime.now().subtract(
          Duration(days: i % 365, hours: i % 24, minutes: i % 60),
        ),
        type: types[i % types.length],
        walletId: wallets[i % wallets.length].id,
        category: categories[i % categories.length],
      ),
    );
  }

  return transactions;
}
